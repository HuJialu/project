...
class ActorNetwork(object):
    ...
    def __init__(self, sess, state_dim, action_dim, learning_rate):
        ...
        # Create the actor network 创建Actor网络
        ...
        # Set all network parameters 设置网络参数
    ...
    def create_actor_network(self):
        with tf.variable_scope('actor', reuse = tf.AUTO_REUSE):
            ...
            # reuse = tf.AUTO_REUSE
            # 输入 None(不做限制) * S_INFO(6) * S_LEN(8)
            # S_INFO (神经网络6个输入) 比特率，缓冲大小，下个视频块大小，带宽（吞吐量+下载时间），剩余视频块数量
            # S_LEN 采用过去8帧（1-8）
            inputs = tflearn.input_data(shape=[None, self.s_dim[0], self.s_dim[1]])

            # 第一层
            # 全连接层 [0, 1)， 128units 8帧比特率/缓冲大小
            split_0 = tflearn.fully_connected(inputs[:, 0:1, -1], 128, activation='relu')
            # actor.reuse_variables()
            # tf.get_variable_scope().reuse_variabacles()
            split_1 = tflearn.fully_connected(inputs[:, 1:2, -1], 128, activation='relu')
            # 1d卷积 128units 4*4kernel 1-8帧下个视频块大小/吞吐量、1-6下载时间
